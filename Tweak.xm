...
static void LoadPreferences() {
    ...
    if(!prefs){
        ...
        prefs = [[NSDictionary dictionaryWithContentsOfFile:path] mutableCopy];
    }
    if(!customFriends){
        NSArray *documents = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *path = [[documents objectAtIndex:0] stringByAppendingPathComponent:@"com.YungRaj.friendmojilist.plist"];
        
        NSDictionary *friendmojiList = [NSDictionary dictionaryWithContentsOfFile:path];
...
static void ScheduleNotifications(NSDictionary *info){
    void *uns = NULL;
    if (IOS_LT(9_0)) {
            // SpringBoardServices
